...
function AgenteConMasMtos(data) {
  ...
  if (data != undefined) {
                      ...
                      for (var m = 0; m < count4; m++) {  //for de mantenimientos
                        if (Object.keys(data[i][aux].Componentes[aux2][aux3].Mantenimientos)[m] != undefined) {
                          ...
                          if(Mto.Encargado){
                            ...
                            if (Agentes.includes(nombre)) {
                              var index = Agentes.indexOf(nombre)
                                CantidadAgentes[index] += 1;
                            }
                            else {
                              Agentes.push(nombre);
                              CantidadAgentes.push(1);
    ...
    var auxAgentes = Ordenar(CantidadAgentes);
    if (auxAgentes[0] != undefined) {
      ArrayOrdenado.push({Agente:Agentes[auxAgentes[0][0]],Cantidad:auxAgentes[0][1]});
    }
    if (auxAgentes[1] != undefined) {
      ArrayOrdenado.push({Agente:Agentes[auxAgentes[1][0]],Cantidad:auxAgentes[1][1]});

    }
    if (auxAgentes[2] != undefined){
      ArrayOrdenado.push({Agente:Agentes[auxAgentes[2][0]],Cantidad:auxAgentes[2][1]});
...
function getAllIndexes(arr, val) {
  ...
  exports.AgenteConMasMtos=AgenteConMasMtos;
